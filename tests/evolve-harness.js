...
// 一致性锁：BUILTIN_MERGE 的键 == rules 描述符里 10 个输出模块（container!=='none'）的 id 集合
(function assertMergeRegistry() {
  ...
  const mergeIds = EVO.getBuiltinMergeIds().slice().sort();
  if (JSON.stringify(outputIds) !== JSON.stringify(mergeIds)) {
    console.error('✗ BUILTIN_MERGE 与输出模块描述符不一致');
    console.error('  描述符输出模块:', outputIds.join(','));
    console.error('  BUILTIN_MERGE :', mergeIds.join(','));
    process.exit(3);
  }
  console.log('✓ BUILTIN_MERGE 分发表与描述符一致（' + mergeIds.length + ' 个输出模块）');
})();
